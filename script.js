...
let myChart = null; // متغير لحفظ الرسم البياني

function toggleAnalysis() {
    const m = document.getElementById('analysisModal');
    m.style.display = (m.style.display === 'block') ? 'none' : 'block';
    if(m.style.display === 'block') {
        updateChart(); // تحديث الرسم عند الفتح
    }
}
...
async function updateChart() {
    ...
    // سنقوم بتوليد بيانات تجريبية تحاكي الواقع (لأن الـ API المجاني قد لا يدعم التاريخ)
    // أو يمكنك جلبها إذا كان اشتراك الـ API يدعم /history
    const labels = ['6 أيام', '5 أيام', '4 أيام', '3 أيام', 'يومين', 'أمس', 'اليوم'];
    const dataPoints = [1.05, 1.04, 1.06, 1.055, 1.07, 1.065, 1.08]; // مثال لبيانات السعر
    ...
    myChart = new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label: `مسار ${f} مقابل ${t}`,
                data: dataPoints,
                borderColor: '#a855f7', // لونك البنفسجي
                backgroundColor: 'rgba(168, 85, 247, 0.2)',
                borderWidth: 3,
                tension: 0.4, // لجعل الخط منحني (Smooth)
                fill: true
            }]
        },
        options: {
